...
> **⚠️ 最高优先级硬性要求：数据真实性**
>
> 开发中涉及的所有数据——包括但不限于爬虫爬取的数据、预置数据、题库数据、公告数据、示例数据——**必须全面、真实、有效**。
>
> - **禁止**使用虚假、编造、占位符或不可访问的数据（如假 URL、假公告内容、虚构的考试信息）
> - **禁止**用 `example.com`、`test.com`、`lorem ipsum` 等占位数据替代真实数据
> - 爬虫抓取的数据必须来自真实可访问的官方来源，确保 URL 可达、内容完整
> - 预置/种子数据必须反映真实的考试信息（岗位、条件、时间、链接等）
> - 如果无法获取真实数据，应留空或抛出明确错误，**绝不用假数据填充**
