...
所有开发中涉及的数据——包括但不限于爬虫爬取数据、预置数据、题库数据、公告数据、示例数据——**必须全面、真实、有效**。

- 禁止使用虚假、编造、占位符或不可访问的数据（假 URL、假公告、虚构考试信息等）
- 禁止使用 `example.com`、`test.com`、`lorem ipsum` 等占位数据
- 爬虫数据必须来自真实可访问的官方来源，URL 可达、内容完整
- 预置/种子数据必须反映真实考试信息（岗位、条件、时间、链接等）
- 无法获取真实数据时，应留空或抛出明确错误，绝不用假数据填充
